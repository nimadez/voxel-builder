...
class Snapshot {
    ...
    createSnapshots() {
        ...
        for (let i = 0; i < shots.length; i++) {
            ...
            // restore previous images on startup
    ...
    saveSnapshots() {
        const total = countLocalStoragesByPrefix(vbstoreSnapshots);
        const zip = new JSZip();
        let count = 0;
        ...
        for (let i = 0; i < total; i++) {
            ...
            if (data) {
                ...
                zip.file(`snapshot${i}.json`, project.save_as_text(data));
                zip.file(`snapshot${i}.png`, img.replace('data:image/png;base64,', ''), { base64: true });
                count++;
            }
        }
        ...
        if (count > 0) {
            zip.generateAsync({ type: "blob" }).then(data => {
                downloadData(data, `snapshots_${getFormattedDate()}.zip`);
            });
        }
    }
    ...
    loadSnapshots(archive) {
        ...
        zip.loadAsync(archive).then(zipData => {
            ...
            Object.keys(zipData.files).forEach(fname => {

                const id = parseInt(fname.split('.')[0].replace('snapshot', ''));

                if (fname.endsWith('.json')) {
                    zip.file(fname).async('string').then(data => {
                        localStorage.setItem(vbstoreSnapshots + 'shot' + id, JSON.parse(data).data.voxels);

                        if (fname === Object.keys(zipData.files)[Object.keys(zipData.files).length - 1]) {
...
document.getElementById('delete_bake_all').onclick = () =>          { pool.clearPool(true) };
