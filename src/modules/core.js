...
class FaceNormalProbe {
    ...
    init() {
        this.probe = CreateBox('facenorm-probe', 1.2, FRONTSIDE, scene);
    ...
    predicate(mesh) {
        return mesh.name === 'facenorm-probe';
    }
    ...
    getNormal(pick, index) {
        ...
        this.probe.position.copyFrom(voxel.position);
        (ui.domDebugPick.checked) ?
            this.probe.layerMask = 0x0FFFFFFF :
            this.probe.layerMask = 0x00000000;
        
        this.ray.origin = pick.ray.origin;
        this.ray.direction = pick.ray.direction;
        ...
        if (res.hit) {
            ...
            if (!this.normal) {
                const voxels = builder.getNearbyVoxels(res.pickedPoint, 0.5);
                if (voxels.length > 0) {
                    for (const v in voxels) {
                        this.normal = this.getFaceNormal(voxels[v].position, res.pickedPoint);
                        if (this.normal)
                            break;
                    }
                } else {
            ...
            // a moment of no surface probe (jumps)
        }
