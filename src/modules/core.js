...
class Ghosts {
    ...
    createSurfaceProbe(voxels) {
        this.disposeSurfaceProbe();
        
        this.surface = new BABYLON.SolidParticleSystem('ghost_surface', scene, { updatable: false, expandable: false, boundingSphereOnly: false });

        this.surface.addShape(this.voxel, voxels.length, { positionFunction: (p, i, s) => {
            p.position.copyFrom(voxels[i].position);
        }});

        this.surface.initParticles();
        this.surface.buildMesh();
        this.surface.computeBoundingBox = true;
        this.surface.mesh.isPickable = true;
        this.surface.mesh.doNotSerialize = true;
        (!ui.domDebugPick.checked) ?
            this.surface.mesh.layerMask = 0x00000000 :
            helper.highlightOverlayMesh(this.surface.mesh, COL_AXIS_X_RGB);
    }
...
class Helper {
    ...
    setWorkplanesPickable(isEnabled) {
        if (helper.isGridPlaneActive)
            this.gridPlane.isPickable = isEnabled;

        if (helper.isWorkplaneActive) {
            this.workplaneX.isPickable = isEnabled;
            this.workplaneY.isPickable = isEnabled;
            this.workplaneZ.isPickable = isEnabled;
        }
    }
...
class Tool {
    ...
    setPickInfo(onHit) {
        ...
        if (index !== undefined) {
            ...
            const norm = faceNormalProbe.getNormal(this.pick, index);
            if (norm) {
...
class RenderTarget {
    ...
    init() {
        this.pickTexture = new BABYLON.RenderTargetTexture('pickTexture', {
            useSRGBBuffer: false, width: window.innerWidth, height: window.innerHeight },
    ...
    read() {
        this.point.x = Math.floor(pointer.x);
        this.point.y = window.innerHeight - Math.floor(pointer.y);
        this.readTexturePixels(engine.engine._gl, this.pickTexture._texture._hardwareTexture.underlyingResource, this.point.x, this.point.y, 1, 1);
    ...
    readTexturePixels(gl, texture, x, y, w, h) {
        ...
        gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0, gl.TEXTURE_2D, null, 0); // unbind
    }
...
class FaceNormalProbe {
    ...
    predicateSPS(mesh) {
        return mesh.name === 'ghost_surface';
    }

    getNormal(pick, index) {
        this.normal = undefined;

        helper.setWorkplanesPickable(false);

        const voxel = builder.voxels[index];

        this.ray = new BABYLON.Ray(pick.ray.origin, pick.ray.direction);

        this.surfaceProbe(voxel);
        
        const res = scene.pickWithRay(this.ray, this.predicateSPS);
        if (res.hit) {
            this.normal = this.getFaceNormal(voxel.position, res.pickedPoint);
            if (!this.normal) {
                // TODO: ineffective ray traps
            }
        } else {
            // no surface probe (jumping between 2 distant voxels)
        }
    ...
    surfaceProbe(voxel) {
        this.surfaceBlock = builder.getNearbyVoxels(voxel.position, 1.5);
        ghosts.createSurfaceProbe(this.surfaceBlock);
    }

    getFaceNormal(position, pickedPoint) {
        const norm = this.calculateFaceNormal(position, pickedPoint);
        const idx = builder.getIndexAtPosition(position.add(norm));

        if (idx === undefined) {
            return norm.clone();
        } else {
            if (!builder.voxels[idx].visible)
                return norm.clone();
            
            const _norm = this.calculateFaceNormal(builder.voxels[idx].position, pickedPoint);
            const _idx = builder.getIndexAtPosition(position.add(_norm));

            if (_idx === undefined) {
                return _norm.clone();
            } else {
                if (!builder.voxels[_idx].visible)
                    return _norm.clone();
    ...
    calculateFaceNormal(position, pickingPoint) {
        const point = pickingPoint.subtract(position);
        ...
        switch (maxIndex) {
            case 0:
                normal = point.x >= 0 ? VEC6_ONE[0] : VEC6_ONE[1];
                break;
            case 1:
                normal = point.y >= 0 ? VEC6_ONE[2] : VEC6_ONE[3];
                break;
            case 2:
                normal = point.z >= 0 ? VEC6_ONE[4] : VEC6_ONE[5];
                break;
        }
...
export function registerRenderLoops() {
    ...
    scene.registerAfterRender(() => {
        if (engine.isRendering) {
            ...
            if (!pointer.isDown && !builder.isWorking) {
...
ui.domDebugPick.onchange = (ev) => {
    (ev.target.checked) ?
        helper.overlayPlane.renderingGroupId = 1 :
