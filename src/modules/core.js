...
class Tool {
    ...
    // 6 dynamic plates slightly enlarged for problematic normals
    // (hard slope camera directions and the gap between faces)
    ...
    normalProbe(pick, index) {
        ...
        for (let i = 1; i < 3; i += 0.1) {
            ...
            pick = scene.pickWithRay(pick.ray);
            if (pick.hit &&
                pick.pickedMesh.name == 'plane' &&
                pick.pickedMesh.name !== 'ghost_spspick') {
...
class RenderTarget {
    ...
    read() {
        this.pointer.x = Math.round(pointer.x);
        this.pointer.y = engine.engine.getRenderHeight() - Math.round(pointer.y);
