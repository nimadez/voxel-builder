...
import { THREE, loaders } from '../modules.js';
...
class Sandbox {
    ...
    init() {
        ...
        this.far = preferences.isMobile ? 2000 : 5000;
        ...
        this.renderer.domElement.onpointerdown = (ev) => {
        ...
        this.renderer.domElement.onpointermove = (ev) => {
            this.mouse.x = (ev.clientX / window.innerWidth) * 2 - 1;
            this.mouse.y = -(ev.clientY / window.innerHeight) * 2 + 1;
        };
        ...
        console.log('Three.js Sandbox v3');
    }

    initPathTracer() {
        this.pt = new PathTracer(this.renderer);
    ...
    createScene() {
        ...
        this.light.shadow.mapSize.width = preferences.isMobile ? 512 : 1024;
        this.light.shadow.mapSize.height = preferences.isMobile ? 512 : 1024;
        ...
        this.mat_shade = new THREE.MeshStandardMaterial({ color: new THREE.Color(0x000000), side: THREE.BackSide });
    ...
    setTonemap(id) {
        this.renderer.toneMapping = toneMappingOptions[id];
    ...
    shot() {
        (sandbox.isRendering) ?
            sandbox.pt.render() :
            sandbox.renderer.render(this.scene, this.camera);

        const uri = sandbox.renderer.domElement.toDataURL('image/png');
